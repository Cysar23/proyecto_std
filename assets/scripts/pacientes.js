...
const tabla_detalle_pacientes = document.getElementById(
  "mostrar_detalle_pacientes"
);
...
const obtener_paciente = () =>
  db.collection("usuarios").doc(uid).collection("datos_pacientes").get();
...
const actualizar_paciente = (id, actualizando) =>
  db
    .collection("usuarios")
    .doc(uid)
    .collection("datos_pacientes")
          ...
          async (event) => {
            if (form.checkValidity() === true) {
              ...
              const enfermera_asiganada =
                form_pacientes["select_enfermeras"].value;
              ...
              const fecha_nacimiento =
                form_pacientes["id_fecha_nacimiento"].value;
              ...
              const id_litiasisRenal =
                form_pacientes["id_litiasisRenal"].checked;
              ...
              const id_cancerTiroides =
                form_pacientes["id_cancerTiroides"].checked;
              const id_cancerMama = form_pacientes["id_cancerMama"].checked;
              const id_cancerCervicoUterino =
                form_pacientes["id_cancerCervicoUterino"].checked;
              const id_cancerProstata =
                form_pacientes["id_cancerProstata"].checked;
              ...
              if (estatus == "agregar") {
                ...
                form_pacientes["btn_eliminar"].classList =
                  "close btn btn-danger btn_eliminar-paciente btn-with";
    ...
    cuando_hay_pacientes((querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        tabla_pacientes.innerHTML += `
                    ...
                    <div class="widget-content-outer">
                      <div class="widget-content-wrapper">
                        <div class="widget-content-left pr-2">
                          <div class="widget-numbers fsize-1 text-success">75%</div>
                        </div>
                        <div class="widget-content-right w-100">
                          <div class="progress-bar-xs progress">
                            <div class="progress-bar bg-success" role="progressbar" aria-valuenow="71" aria-valuemin="0" aria-valuemax="100" style="width: 75%;">
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <div class=" d-flex justify-content-center align-items-center">
                    <button type="button" id="" class="btn btn-info btn-sm btn-detalle mr-2" data-id="${paciente.id}" data-toggle="modal" data-target="#modal_pacientes_detalle" >Detalle</button>
          ...
          btn.addEventListener("click", async (e) => {
            ...
            tabla_detalle_pacientes.innerHTML += `
                        <tr>
                            ...
                            <td class="text-center">
                                <button type="button" id="" class="btn btn-primary btn-sm btn-ver" onclick="consulta()" data-id="${paciente.id}" data-toggle="modal"  data-target="#modal_expediente">Ver</button>
            ...
            btnsVer.forEach((btn) => {
              btn.addEventListener("click", async (e) => {
                console.log("BTN VER ID: ",e.target.dataset.id);
                //const id_nombre_paciente_exp = document.getElementById(
                 // "id_nombre_paciente_exp"
                //);
                const doc = await editarPaciente(e.target.dataset.id);
                const exp = doc.data();
                //console.log(exp);
        ...
        btnsEdit.forEach((btn) => {
          btn.addEventListener("click", async (e) => {
            ...
            form_pacientes["btn_eliminar"].classList =
              "mb-2 mr-2 btn-pill btn btn-danger btn_eliminar-paciente btn-width";

            form_pacientes["estado"].value = paciente.estado;

            form_pacientes["select_enfermeras"].value =
              paciente.enfermera_asiganada;
            ...
            form_pacientes["id_dislipidemia"].checked =
              paciente.id_dislipidemia;
            ...
            form_pacientes["id_renalCronica"].checked =
              paciente.id_renalCronica;
            ...
            form_pacientes["id_CancerPulmon"].checked =
              paciente.id_CancerPulmon;
            ...
            form_pacientes["id_cancerCervicoUterino"].checked =
              paciente.id_cancerCervicoUterino;
      ...
      btnsEliminar.forEach((btn) => {
        btn.addEventListener("click", async (e) => {
          ...
          let doc_id;
          const id_disp = await db
            .collection("usuarios")
            .doc(uid)
            .collection("datos_dispositivos")
            .where("paciente", "==", form_pacientes["id_nombre"].value)
            .get();
          id_disp.forEach((doc) => {
          ...
          let conf = confirm(
            "Esta seguro de eliminar a " + form_pacientes["id_nombre"].value
          );
          if (conf == true) {
            conf = confirm(
              "SI ELIMINA AL PACIENTE, ELIMINARA EL DISPOSITIVO ASIGNADO"
            );
            if (conf == true) {
...
function asignar_enfermeras() {
  ...
  cuando_hay_enfermeras((querySnapshot) => {
    ...
    if (!cant_enfermeras) {
      select_enfermeras.innerHTML = `<option>No hay enfermeras</option>`;
    }
...
function valida(id_cedula) {
  ...
  if (c[10] == dp) {
    ...
    document
      .getElementById("id_cedula_validar")
      .setAttribute("style", "color: green");
  } else {
    document.getElementById("id_cedula_validar").innerHTML =
      "<p>Incorrecta </p>";
    document
      .getElementById("id_cedula_validar")
...
async function agregarNota() {
  ...
  await actualizar_paciente(idPaciente, {
    expediente: firebase.firestore.FieldValue.arrayUnion(nota),
  });
...
function buscarNotas() {
  ...
  // Convertir a minúsculas para hacer comparaciones insensibles a mayúsculas/minúsculas
  const fechaBuscarMin = fechaBuscar.toLowerCase();
  const contenidoBuscarMin = contenidoBuscar.toLowerCase();

  const notasFiltradas = notas.filter(
    (nota) =>
      nota.fecha.includes(fechaBuscarMin) &&
      nota.contenido.toLowerCase().includes(contenidoBuscarMin)
  );
...
async function actualizarListaNotas() {
  const doc = await editarPaciente(idPaciente);
  const data_pacientes = doc.data();
  console.log(data_pacientes.expediente);
  mostrarNotas(data_pacientes.expediente);
}

async function mostrarNotas(notasArray) {
  ...
  if (notasArray) {
    notasArray.forEach((nota) => {
      ...
      notaHTML.innerHTML = `
             <div class="card-body">
               <h6 class="card-subtitle mb-2 text-muted">${nota.fecha} a las ${nota.hora}</h6>
...
async function consulta() {
  const doc = await editarPaciente(idPaciente);
  const data_pacientes = doc.data();
  const data_expediente = data_pacientes.expediente;
  notas = data_expediente;
  mostrarNotas(notas);
}
