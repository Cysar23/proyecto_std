...
const tabla_detalle_enfermeras = document.getElementById(
  "tabla_detalle_enfermeras"
);
const tabla_detalle_pacientes = document.getElementById(
  "tabla_detalle_pacientes"
);
...
const actualizar_enfermera = (id, actualizando) =>
  db
    .collection("usuarios")
    .doc(uid)
    .collection("datos_enfermeras")
        ...
        form.addEventListener(
          ...
          async (event) => {
            if (form.checkValidity() === true) {
              ...
              form_enfermeras["btn_agregar"].disabled = true;
              ...
              const fecha_nacimiento =
                form_enfermeras["id_fecha_nacimiento"].value;
              ...
              const id_fecha_ingreso =
                form_enfermeras["id_fecha_ingreso"].value;
              ...
              if (estado_btn == "agregar") {
                if (password != password_2) {
                  ...
                  form_enfermeras["btn_agregar"].disabled = false;
                  form_enfermeras.reset();
                  alert("ENFERMERA AGREGADA");
                }
              } else if (estado_btn == "editar") {
                await actualizar_enfermera(id, {
                  email: email,
                  nombre: nombre,
                  apellido: apellido,
                ...
                form_enfermeras["btn_eliminar"].classList =
...
window.addEventListener("DOMContentLoaded", async (e) => {
  auth.onAuthStateChanged(function (user) {
    ...
    cuando_hay_enfermeras((querySnapshot) => {
      const cantidad_enfermeras = document.getElementById(
        "cantidad_enfermeras"
      );
      ...
      querySnapshot.forEach((doc) => {
        ...
        } else if (enfermera.estado === "Activo") {
          //A esto hay que ponerle True o False en la db
        ...
        btnsDetalle.forEach((btn) => {
          btn.addEventListener("click", async (e) => {
            ...
            idEnfermera = e.target.dataset.id;
            ...
            tabla_detalle_enfermeras.innerHTML += `
                        <tr>
                            ...
                            <td class="text-center">
                                <button type="button" id="" class="btn btn-primary btn-sm btn-ver" onclick="consulta()"  data-id="${enfermera.id}" data-toggle="modal"  data-target="#modal_expediente">Ver</button>
            ...
            cuando_hay_pacientes((querySnapshot) => {
              ...
              querySnapshot.forEach((doc) => {
                ...
                const id_nombreApellido =
                  data_enfermeras.nombre + " " + data_enfermeras.apellido;
                if (
                  id_nombreApellido == data_pacientes.enfermera_asiganada &&
                  data_pacientes.estado == "Activo"
                ) {
        ...
        btnsEdit.forEach((btn) => {
          btn.addEventListener("click", async (e) => {
            ...
            form_enfermeras["btn_eliminar"].classList =
              "mb-2 mr-2 btn-pill btn btn-danger btn_eliminar-paciente btn-lg btn-block";
            ...
            form_enfermeras["id_nacionalidad"].value =
              enfermera.id_nacionalidad;
            ...
            form_enfermeras["id_nota"].value = enfermera.id_nota;
            form_enfermeras["id_nota"].disabled = campStatus;
      ...
      btnsEliminar.forEach((btn) => {
        btn.addEventListener("click", async (e) => {
          let conf = confirm(
            "Esta seguro de eliminar a " + form_enfermeras["id_nombre"].value
          );
          if (conf == true) {
            conf = confirm("Seguro?");
            if (conf == true) {
...
function limpiar_form_enfermeras() {
  ...
  modalLabel.innerText = "Agregar nueva enfermera";
  id = "";
  form_enfermeras["email"].disabled = false;
...
async function agregarNota() {
  ...
  await actualizar_enfermera(idEnfermera, {
...
async function actualizarListaNotas() {
  const doc = await editarEnfermera(idEnfermera);
  const data_enfermera = doc.data();
  console.log(data_enfermera.expediente);
  mostrarNotas(data_enfermera.expediente);
}
...
async function consulta() {
  const doc = await editarEnfermera(idEnfermera);
  const data_enfermera = doc.data();
  console.log(data_enfermera)
  const data_expediente = data_enfermera.expediente;
